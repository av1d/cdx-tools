...
def setup():
    ...
    if args['collapse'] == None:  # set default "digest"
...
def cdxToDict(cdx_response):
    ...
    for line in cdx_response:
        ...
        keys = d[0]     # get list of keys

        cdx_dict = {}   # turn keys into dictionary
        for i in keys:
            cdx_dict[i] = None

        val = d[count]  # get first line of values
...
def fetchResponse():
    ...
    if args["url"] != None:  # create a unique filename
        ...
        # turn response into valid JSON, one dictionary per line
