...
def setArgs():
    ...
    parser.add_argument(
        ...
        required=False,
        help=
                "Specifies type of download scripts to generate from results,\n" \
                "if any. Valid options: curl, wget.\n" \
                "You must specify an output script file with --outfile.\n"
                + sep(),
    )
    parser.add_argument(
        '--live-dangerously',
        ...
        required=False,
        help=
                "Disable percent-encoding on output filenames generated by\n" \
                "the script generator and use the literal path regardless\n" \
                "of which characters it contains. (No sanitation).\n"
               + sep(),
    )
    parser.add_argument(
        '-v',
    ...
    if (
         ...
         args['outfile'] == None
            ...
            print(
                    "Error: You must use --outfile when " \
                    "using --gen\n"
            )
            sys.exit(1)
    ...
    if (
         ...
         args['field'] != None
            ...
            print(
                    "Error: You cannot use --gen when using --field.\n" \
                    "You can use --field with --outfile to generate JSON\n"\
                    "then process that JSON with --gen to make a script.\n"
            )
            sys.exit(1)
    ...
    global bashgen         # bool. for writing scripts
    global genScript       # str.  for writing scripts
...
def generateOutput(url_string, timestamp):
    ...
    percent_encode = quote(str(remotePath))
    encoded_path = Path(percent_encode)
    final_path = str(encoded_path)
